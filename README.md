...
3. **Asistente de arranque**: `pnpm setup` detecta tu plataforma, valida los prerrequisitos (Python, toolchain nativa) y ejecuta `pnpm install`. Al finalizar permite lanzar `pnpm dev` o iniciar servicios individuales (`api`, `sfu`, `companion`) desde un único flujo guiado.
